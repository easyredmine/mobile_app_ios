...
#define ISSUE_BASE_DETAIL_SECTION 1
#define ISSUE_TIME_SECTION 2
#define ATTACHMENTS_SECTION 4
#define CUSTOM_FIELDS_SECTION 5


@interface NewIssueViewController() <UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIActionSheetDelegate> {
}
...
-(void)viewDidLoad
{
    ...
    self.title = NSLocalizedString(@"newTask", @"");
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    switch (section) {
        ...
        case ISSUE_BASE_DETAIL_SECTION: return 4 + (self.pickerIndexPath && self.pickerIndexPath.section == ISSUE_BASE_DETAIL_SECTION);
        case ISSUE_TIME_SECTION: return 5 + (self.pickerIndexPath && self.pickerIndexPath.section == ISSUE_TIME_SECTION);
        case 3: return 0;//return 1;
        case ATTACHMENTS_SECTION: return 1 + self.issue.fileUploads.count; //TODO: +current attachments
        case CUSTOM_FIELDS_SECTION: return self.issue.customFields.count + (self.pickerIndexPath && self.pickerIndexPath.section == 5);
        default: return 0;
    }
}
...
-(BOOL)canOpenPickerForModelIndexPath:(NSIndexPath *)modelIndexPath
{
    ...
    switch (modelIndexPath.section) {
        case ISSUE_BASE_DETAIL_SECTION:
        ...
        case ISSUE_TIME_SECTION:
            switch (modelIndexPath.row) {
                case 0: // Milestone
                case 3: // Estimated time
                    return NO;
                case 4: // Category
                    return self.issue.availableCategories.count;
...
-(void)configurePickerCell:(EditPickerCell *)cell forModelIndexPath:(NSIndexPath *)modelIndexPath
{
    switch (modelIndexPath.section) {
        ...
        case CUSTOM_FIELDS_SECTION: //percent
        {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.pickerIndexPath && self.pickerIndexPath.section == indexPath.section && self.pickerIndexPath.row <= indexPath.row) {
        ...
        if(self.pickerIndexPath.row == indexPath.row){
            ...
            } else if (modelIndexPath.section == ISSUE_BASE_DETAIL_SECTION) {
                // Priority and assignee
                EditPickerCell *pickerCell = [tableView dequeueReusableCellWithIdentifier:EditPickerCellID forIndexPath:indexPath];
                [self configurePickerCell:pickerCell forModelIndexPath:modelIndexPath];
                return pickerCell;
            } else if (modelIndexPath.section == ISSUE_TIME_SECTION) {
    ...
    switch (modelIndexPath.section) {
        case PROJECT_SECTION: // Base Project info
        {
            switch (modelIndexPath.row) {
                case 0:
                {
            ...
            break;
        case ISSUE_BASE_DETAIL_SECTION: // Priority and assignee
        {
            switch (modelIndexPath.row) {
                case 0: // Tracker
                {
            ...
            break;
        case ISSUE_TIME_SECTION: // Times, milestone and category
        {
            switch (modelIndexPath.row) {
                case 0: // Milestone
                {
        ...
        case 3:
        {
            ...
            EditTextFieldCell *parentCell = [tableView dequeueReusableCellWithIdentifier:EditTextFieldCellID forIndexPath:indexPath];
            parentCell.textField.keyboardType = UIKeyboardTypeNumberPad;
            parentCell.textField.placeholder = NSLocalizedString(@"new_task_none", @"");
            parentCell.leftLabel.text = NSLocalizedString(@"new_task_parent_task", @"");
            parentCell.textField.text = self.issue.parent[@"value"];
            
            @weakify(self)
            [[[parentCell.textField.rac_textSignal skip:1] takeUntil:parentCell.rac_prepareForReuseSignal]  subscribeNext:^(NSString *x) {
                @strongify(self);
                NSNumber *n = [[[self class] floatFormatter] numberFromString:x];
                self.issue.estimatedHours = n;
            }];
...
{
    switch (section) {
        case 4: return NSLocalizedString(@"attachments", @"");
        case 5: if(self.issue.customFields.count > 0) return NSLocalizedString(@"task_detail_custom_fields", @"");
        default: return nil;
    }
}
...
{
    RACSignal *networkRequest = [[DataService sharedService] createIssue:self.issue];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // First section and subject and description cell
    if (indexPath.section == PROJECT_SECTION && (indexPath.row == 1 || indexPath.row == 2)) {
